---
title: "Reactivate Membership"
api: "POST /organizations/{organizationId}/memberships/{membershipId}/reactivate"
description: "Reactivate a membership."
---

Reactivates a membership by its ID.

<Info>Requires `memberships:reactivate` permission.</Info>

## Headers

<ParamField header="Authorization" type="string" required>
  Bearer token.
</ParamField>

## Path Parameters

<ParamField path="organizationId" type="string" required>
  The organization UUID.
</ParamField>

<ParamField path="membershipId" type="string" required>
  The membership ID.
</ParamField>

## Response

<ResponseField name="membershipId" type="string">
  The membership unique identifier.
</ResponseField>

<ResponseField name="userId" type="string">
  The user ID.
</ResponseField>

<ResponseField name="organizationId" type="string">
  The organization UUID.
</ResponseField>

<ResponseField name="role" type="object">
  The assigned role.
</ResponseField>

<ResponseField name="status" type="string">
  The membership status.
</ResponseField>

<ResponseField name="createdAt" type="string">
  Creation timestamp (ISO 8601).
</ResponseField>

<ResponseField name="updatedAt" type="string">
  Last update timestamp (ISO 8601).
</ResponseField>

<ResponseExample>
```json 200
{
  "membershipId": "om_01HQZC8G2T9YX5J8VQBW3KPQR7",
  "userId": "019525fd-56a8-7db4-8c3e-2a1b4d6f8e0c",
  "organizationId": "019525fd-4c38-7e30-a5c1-b6e3f4d8a9c2",
  "role": {
    "slug": "admin"
  },
  "status": "ACTIVE",
  "createdAt": "2026-01-15T10:00:00.000Z",
  "updatedAt": "2026-03-25T14:00:00.000Z"
}
```
```json 400
{
  "statusCode": 400,
  "message": "Bad Request Error",
  "errors": [{"message": "required", "path": "field"}]
}
```
```json 401
{
  "statusCode": 401,
  "message": "Unauthorized"
}
```
```json 403
{
  "statusCode": 403,
  "message": "Forbidden"
}
```
```json 404
{
  "statusCode": 404,
  "message": "organization.not_found",
  "resourceId": "019525fd-4c38-7e30-a5c1-b6e3f4d8a9c2"
}
```
```json 500
{
  "statusCode": 500,
  "message": "Internal Server Error"
}
```
</ResponseExample>

## Error Codes

| Status | Code | Description |
|--------|------|-------------|
| `400` | `validation_error` | Invalid input or missing required fields |
| `401` | `unauthorized` | Missing or invalid authentication |
| `403` | `forbidden` | Insufficient permissions |
| `404` | `organization.not_found` | Organization not found |
| `404` | `membership.not_found` | Membership not found |
| `500` | `internal_server_error` | An unexpected error occurred |
